...
const fetchCompanyDetailsID = async () => {
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data) && response.data.length > 0) {
      ...
      response.data.company_detail_id
    ) {
...
const saveAsDraft = () => {
  if (!formData.value.unit_in_capacity || !formData.value.waste_water_source) {
    this.$toast.error(
      "Beberapa data penting belum diisi, tetapi data akan disimpan sebagai draft."
    );
  } else {
    this.$toast.success("Data berhasil disimpan sebagai draft.");
  }

  axios
    .post("/api/save-draft", formData.value)
    .then(() => {
      this.$toast.success("Draft berhasil disimpan.");
    })
    .catch((error) => {
      this.$toast.error("Terjadi kesalahan saat menyimpan draft.");
      console.error(error);
    });
};
...
const chemicalDetails = ref([
  { chemicals_used: "", use_of_chemicals: "", unit_in_use_of_chemicals: "" },
]);

const addChemical = () => {
  chemicalDetails.value.push({
    chemicals_used: "",
    use_of_chemicals: "",
        ...
        <div class="row">
          <div class="col-lg-10 mx-auto">
            ...
            <div class="row">
              ...
              <div class="col-md-8">
                <div class="form-group">
                  ...
                  <select class="form-control" v-model="formData.company_licence_id">
            ...
            <div class="row">
              <div class="col-md-6">
                <div class="form-group">
                  <label class="col-form-label">Debit Air Limbah Yang Diijinkan</label>
                  <input
                    type="text"
                    class="form-control"
            ...
            <div>
              <table class="table table-bordered mb-4">
                ...
                <tbody>
                  <tr v-for="(recycle, index) in recycleDetails" :key="index">
                    <td>{{ index + 1 }}</td>
                    <td>
                      <input
                        type="text"
                    ...
                    <td>
                      <button @click="removeRecycle(index)" class="btn btn-danger">
                        -
                      </button>
                      <button @click="addRecycle" class="btn btn-success">+</button>
                    </td>
                  </tr>
                </tbody>
              </table>
            ...
            <div>
              <table class="table table-bordered mb-4">
                ...
                <tbody>
                  <tr v-for="(chemical, index) in chemicalDetails" :key="index">
                    <td>{{ index + 1 }}</td>
                    <td>
                      <input
                        type="text"
                    ...
                    <td>
                      <button @click="removeChemical(index)" class="btn btn-danger">
                        -
                      </button>
            ...
            <div class="col-md-6">
              <div class="form-group">
                <label class="col-form-label">Tempat Penampungan Lumpur IPAL</label>
                <input
                  type="text"
                  class="form-control"
            ...
            <button @click="saveAsDraft" class="btn btn-primary">Simpan</button>
            <button @click="createIpalEntry" class="btn btn-success">
              Simpan dan Submit
            </button>
