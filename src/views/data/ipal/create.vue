...
// Function to fetch company licenses for the dropdown
...
// Call fetchCompanyLicences when the component mounts
...
const createIpalEntry = async () => {
  ...
  try {
    ...
    formData.value.waste_discharge_measuring_instrument_outlet =
      ...
      formData.value.company_detail_id = formData.value.company_licence_id;
    await axios.post(
    ...
    Swal.fire('Berhasil', 'Data IPAL berhasil ditambahkan.', 'success')
    router.push('/Data/Ipal')
  } catch (error) {
    console.error('Error creating IPAL entry:', error)
    Swal.fire('Error', 'Gagal menambahkan data IPAL.', 'error')
  } finally {
    loader.hide()
  }
}
